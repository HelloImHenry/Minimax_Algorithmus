...
depth = 3
#Farbe Maximieren [VERÄNDERBAR]
colorMax = "#0000FF"
#Farbe Minimieren [VERÄNDERBAR]
colorMin = "#00FF00"
#Farbe ausgewählter Pfad [VERÄNDERBAR]
colorSelectedPath = "#EE6AA7"
...
def drawCircleLine(layerDepth, amountOfCircles):
    ...
    for x in range(amountOfCircles):
        ...
        if layerDepth%2 == 1:
            id = cv.create_oval(xPosLeft,yPosTop,xPosRight,yPosBottom,width = 2,fill = colorMax)
        else:
            id = cv.create_oval(xPosLeft,yPosTop,xPosRight,yPosBottom,width = 2,fill = colorMin)
...
def drawSelectedLines():
    ...
    for n in (tree[::-1]):
        ...
        if n.parentNode == parentNode:
            drawLine(posArray[parentNode[1]][parentNode[0]],posArray[n.childNode[1]][n.childNode[0]],colorSelectedPath)
